...
| `ChromiumLocalStorageTests` | The LevelDB reader, which is the only place Pulse parses somebody else's binary format: one origin's entries and nobody else's, the later write winning and a deletion counting as one, a record reassembled across 32KB blocks, block padding skipped rather than read as the end of the file (a real bug, which dropped everything after the first padded block), both of Chromium's text encodings, the prefix range that lets whole blocks go unread, and Snappy — an overlapping copy, a long literal, and a stream that promises more than it delivers. The store is built by hand; nobody's profile is committed. [providers/devin.md](providers/devin.md) |
| `SpendSummaryTests` | The Token spend pane's arithmetic: a span as a calendar window rather than each ledger's last few rows (a real bug — "the last 7 days" drew nine bars), quiet days padded so the chart stays a calendar, a ledger that cannot be priced left out of a priced total, agents and models ranked, a model used by two agents naming both, streaks counted from the end of the series, the peak hour read from the quarter-hour buckets, sessions filtered by when they *ended*, projects rolled up with no "other" bucket, and the day table sorted by any column both ways. [token-spend.md](token-spend.md) |
...
| `SessionTitleTests` | What a transcript says about itself: a title cut to one line, a command envelope or the sandbox caveat refused (either would name most sessions the same thing), both shapes of message body, and the folder name used only where no `cwd` was stated. [token-spend.md](token-spend.md) |
