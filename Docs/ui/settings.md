...
Added accounts have a **Sign in again** control; successful reauthentication replaces credentials in the selected slot, preserving its name and display preferences. They do not show ambient CLI source controls that their fetch ignores. A cancelled sign-in or an account removed while sign-in is pending is not written back. One sign-in runs at a time, and its **Cancel**, device code and error rows appear only on the panes of the provider it was started for; other multi-account panes show a disabled **Sign in…** until it finishes. (A Codex device code was once shown on the Claude Code pane, where it read as Claude Code asking for it — Claude Code has no device flow.)
