...
Pulse's catalog recognizes **53 agent sources**: the seven clients it read on a real machine before this change (`claudeCode`, `codex`, `openCode`, `kiloCLI`, `grok`, `kimiCLI`, `devinCLI`) and **46** added from second-hand format specifications of a compatibility target (Tokscale 4.17.0). Some sources require exports or do not report usable token counters; the table below distinguishes them. No upstream code, test or fixture is vendored; each added reader was written independently from file locations, field meanings, units and event identities, with original synthetic test data. The new readers have not been verified against live client stores.
...
The VS Code task logs are `ui_messages.json` (`say == "api_req_started"`), each carrying the four counts in an inner JSON string. Cherry Studio appends the same API call several times with one `requestId`; the reader merges those by field-wise maximum so a streamed snapshot is not counted as new usage. Command Code is a tree and only the branch ending at the last entry is counted, so a `/rewind` does not bill an abandoned reply. Tencent's **JSONL transcript is the only reconcilable channel**: when it is present the extension log and the aggregate database are not returned, because they share no identity with its message ids and adding them would double count; when that excluded fallback held records, the transcript records are marked `isPartial`. With no transcript, the fallback stands alone — and a mirrored log line may then be counted twice, which is preferred to folding two real same-second requests into one.
...
| `devin-desktop` | `~/Library/Application Support/Devin/User/acp-events/*.ndjson`, `~/.config/devin/User/acp-events` | NDJSON counters; the CLI DB supplies lookup metadata only and does not establish Desktop presence | spec |
...
- **Devin Desktop vs Devin CLI**: the Desktop NDJSON resolves its session/model/workspace from the CLI DB by title; the two are separate sources and are not merged by a dedup key.
...
- **`antigravity-cli`** — `#1` is not counted (its "fixed system prompt" reading is reverse-engineered, not established) and the unknown `#9.#10` bytes are never decoded into a time; every record is partial because no whole-usage total reconciles the counted fields.
