...
- **明確な使用量の推定**：直近 7 日を初期表示し、選んだ期間を記憶します。コストは公開 API 価格で算出し、サブスクリプションの請求額ではありません。価格が不明な場合やトークン数の集計が不完全な場合、時刻の詳細が分からない場合はその旨を表示します。トークン数を記録しないソースを、使用量ゼロとして扱うことはありません。
...
- **プライバシー第一**：Pulse のサーバーもアカウントもテレメトリーもありません。Pulse はすでに使っているプロバイダに接続し、トークン消費ペインのために [models.dev](https://models.dev) から公開モデル価格を取得し、GitHub/Sparkle でアプリの更新を確認します。macOS のシステムプロキシ設定は引き続き適用されます。
...
- **Pulse のバックエンドはなし**：Pulse のサーバー、アカウント、テレメトリーはありません。アプリはすでに使っているプロバイダと直接通信し、独自のプロキシを挟みません。また、トークン消費ペインのために [models.dev](https://models.dev) から公開モデル価格を取得し、GitHub/Sparkle でアプリの更新を確認します。macOS のシステムプロキシ設定は引き続き適用されます。
...
- **ローカルの利用記録**：Pulse はトークン数と、タイトルや作業ディレクトリといったセッション情報を得るために、トランスクリプト・データベース・エクスポートを読み取ります。これらの記録には会話のテキストが含まれることがありますが、処理はあなたの Mac 上で完結し、記録がアップロードされることはありません。Pulse は使用量の把握のためにソースファイルやシェル履歴をスキャンしません。
